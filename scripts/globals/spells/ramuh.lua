...
function onSpellCast(caster,target,spell)
    caster:spawnPet(dsp.pet.id.RAMUH)
    ...
    if (pet ~= nill) then
        pet:addStatusEffect(dsp.effect.RAMUH_S_FAVOR, avatarBuff, 3, 3000);
        pet:addStatusEffect(dsp.effect.ENTHUNDER,enspellPower,0,3000);
    end
    return 0
end
