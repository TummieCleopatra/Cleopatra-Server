...
function onSpellCast(caster,target,spell)
    caster:spawnPet(dsp.pet.id.IFRIT)
    ...
    if (pet ~= nill) then
        pet:addStatusEffect(dsp.effect.IFRIT_S_FAVOR, avatarBuff, 3, 3000);
        pet:addStatusEffect(dsp.effect.ENFIRE,enspellPower,0,3000);
    end
    return 0
end
