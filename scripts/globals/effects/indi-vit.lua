...
function onEffectTick(target,effect)
    if (target:getObjType() == dsp.objType.PC) then
        target:forMembersInRange(10, function(member)
            if (member:hasStatusEffect(dsp.effect.INDI_VIT) == false) then
                member:addStatusEffect(dsp.effect.VIT_BOOST_III, effect:getPower(), 0, 6)
            end
        end)
    else
        ...
        for i,members in pairs(nearbyChars) do
            if (members:getObjType() == dsp.objType.PC and members:getAllegiance() == target:getAllegiance()) then
                members:addStatusEffect(dsp.effect.VIT_BOOST_III, effect:getPower(), 0, 6)
            end
        end
    end
