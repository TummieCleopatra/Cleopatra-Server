...
function onEffectTick(target,effect)
    if (target:getObjType() == dsp.objType.PC) then
        target:forMembersInRange(10, function(member)
            if (member:hasStatusEffect(dsp.effect.INDI_FOCUS) == false) then
                member:addStatusEffect(dsp.effect.MAGIC_ACC_BOOST_II, effect:getPower(), 0, 6)
            end
        end)
    else
        ...
        for i,members in pairs(nearbyChars) do
            if (members:getObjType() == dsp.objType.PC and members:getAllegiance() == target:getAllegiance()) then
                members:addStatusEffect(dsp.effect.MAGIC_ACC_BOOST_II, effect:getPower(), 0, 6)
            end

            local party = members:getParty()
            for i, trust in ipairs(party) do
                if (trust:getObjType() == dsp.objType.TRUST) then
                    trust:addStatusEffect(dsp.effect.MAGIC_ACC_BOOST_II, effect:getPower(), 0, 6)
                end
            end
        end
    end
