...
function onMobDeath(mob, player, isKiller)
    ...
    local gate = 17084691; -- Dormant Rampart
	...
	if (player:getObjType() ~= dsp.objType.PC) then
		...
		if (level >= 70 and level <= 75 and GetMobAction(nm1) == 0 and GetMobAction(nm2) == 0 and GetMobAction(nm3) == 0 and GetMobAction(nm4) == 0 and gear == 1) then
            if (math.random(0,30) < chance) then
				...
				SpawnMob(17084713):setPos(30,0,9);
            end
		else
		...
		if (plvl >= 70 and plvl <= 75 and GetMobAction(nm1) == 0 and GetMobAction(nm2) == 0 and GetMobAction(nm3) == 0 and GetMobAction(nm4) == 0 and gear == 1) then
            if (math.random(0,99) < chance) then
	            if (player:getObjType() == dsp.objType.PC) then
                    player:PrintToPlayer("A gateway has spawned...",0x15);
	            end
				GetNPCByID(gate):setLocalVar(gate, timestamp); -- Timestamp NPC
				player:setLocalVar(gate,timestamp); -- Timestamp the Player
	            GetNPCByID(gate):setPos(killx+1,killy,killz);
                GetNPCByID(gate):showNPC(60);
				SpawnMob(17084713):setPos(30,0,9);
	        end
