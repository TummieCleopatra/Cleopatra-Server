...
function onMobEngaged(mob,target)
    target:PrintToPlayer("??? : All shall perish by the hands of the Great Curilla.", 0xD);
end;
...
function onMobFight(mob,target)
    ...
    if (isweak ~= 1) and (trusttalk ~= 1) and (hp < 60) then
        if (target:getObjType() == dsp.objType.PET or target:getObjType() == dsp.objType.TRUST) then
			...
			master:PrintToPlayer(string.format("(Curilla) %s, we must do something to make this imposter show its true self!", target:getName()),0xF);  
			mob:setLocalVar("TrustTalk",1);
	    elseif (target:getObjType() == TYPE_PC) then
	        target:PrintToPlayer(string.format("(Curilla) %s, we must do something to make this imposter show its true self!", target:getName()),0xF);  
		    mob:setLocalVar("TrustTalk",1);			
		end
	end
...
local mammett = 17347091;	
	
	
	SpawnMob(mammett):setPos(killx,killy,killz);
    GetMobByID(17347091):updateClaim(killer);	
end;