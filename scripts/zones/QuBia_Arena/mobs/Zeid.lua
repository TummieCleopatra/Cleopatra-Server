...
function onMobEngaged(mob,target)
    target:PrintToPlayer("??? : This fight has already be decided before I even drew my sword...you shall perish!", 0xD);
end;
...
function onMobFight(mob,target)
    ...
    if (isweak ~= 1) and (trusttalk ~= 1) and (hp < 60) then
        if (target:getObjType() == dsp.objType.PET or target:getObjType() == dsp.objType.TRUST) then
			...
			master:PrintToPlayer(string.format("(Zeid) %s, we must strike this imposter down to reveal its true self!", target:getName()),0xF);
			mob:setLocalVar("TrustTalk",1);
	    elseif (target:getObjType() == dsp.objType.PC) then
	        target:PrintToPlayer(string.format("(Zeid) %s, we must strike this imposter down to reveal its true self!", target:getName()),0xF);
		    mob:setLocalVar("TrustTalk",1);
		end
	end
