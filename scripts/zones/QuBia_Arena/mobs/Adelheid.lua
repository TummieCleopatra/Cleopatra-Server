...
function onMobEngaged(mob,target)
    target:PrintToPlayer("??? : I shall defeat you with the vast knowledge that I have accumulated!!", 0xD);
end;
...
function onMobFight(mob,target)
    ...
    if (isweak ~= 1) and (trusttalk ~= 1) and (hp < 60) then
        if (target:getObjType() == dsp.objType.PET or target:getObjType() == dsp.objType.TRUST) then
			...
			master:PrintToPlayer(string.format("(Adelheid) %s, we must strike this imposter down to reveal its true self!", target:getName()),0xF);  
			mob:setLocalVar("TrustTalk",1);
	    elseif (target:getObjType() == TYPE_PC) then
	        target:PrintToPlayer(string.format("(Adelheid) %s, we must strike this imposter down to reveal its true self!", target:getName()),0xF);  
		    mob:setLocalVar("TrustTalk",1);			
		end
	end
