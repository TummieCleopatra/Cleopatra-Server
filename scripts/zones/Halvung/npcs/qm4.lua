...
function onTrigger(player,npc)
    if (player:hasKeyItem(dsp.ki.GOLD_COLORED_SEAL)) then
	    if (player:getFreeSlotsCount() == 0) then
		    player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED,2586);
	    else
	        player:delKeyItem(dsp.ki.MAROON_SEAL);
			player:messageSpecial(ID.text.KEYITEM_LOST,dsp.ki.GOLD_COLORED_SEAL);
		    player:addItem(2586,1);
			player:messageSpecial(ID.text.ITEM_OBTAINED,2586);
		end
    else
