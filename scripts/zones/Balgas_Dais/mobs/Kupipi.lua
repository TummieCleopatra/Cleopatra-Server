...
function onMobEngaged(mob,target)
    target:PrintToPlayer("??? : I am Kupipi...Legendary Mage of Heaven's Tower!  All shall bow down before me!!!", 0xD);
end;
...
function onMobFight(mob,target)
    ...
    if (isweak ~= 1) and (trusttalk ~= 1) and (hp < 60) then
        if (target:getObjType() == dsp.objType.PET or target:getObjType() == dsp.objType.TRUST) then
			...
			master:PrintToPlayer(string.format("(Kupipi) %s, this imposter is sullying my great name...we must do something!", target:getName()),0xF);
			mob:setLocalVar("TrustTalk",1);
	    elseif (target:getObjType() == dsp.objType.PC) then
	        target:PrintToPlayer(string.format("(Kupipi) %s, this imposter is sullying my great name...we must do something!", target:getName()),0xF);
		    mob:setLocalVar("TrustTalk",1);
		end
	end
