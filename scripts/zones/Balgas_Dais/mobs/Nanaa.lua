...
function onMobEngaged(mob,target)
    target:PrintToPlayer("??? : Get Rrready to be torrrn to pieces by the great Theif Nanaa Mihgo!!", 0xD);
end;
...
function onMobFight(mob,target)
    ...
    if (isweak ~= 1) and (trusttalk ~= 1) and (hp < 60) then
        if (target:getObjType() == dsp.objType.TRUST) then
            ...
            master:PrintToPlayer(string.format("(Nanaa) %s, we must do something to dispose of this cat imposter!", target:getName()),0xF);
            mob:setLocalVar("TrustTalk",1);
        elseif (target:getObjType() == dsp.objType.PC) then
            target:PrintToPlayer(string.format("(Nanaa) %s, we must do something to dispose of this cat imposter!", target:getName()),0xF);
            mob:setLocalVar("TrustTalk",1);
        end
    end
